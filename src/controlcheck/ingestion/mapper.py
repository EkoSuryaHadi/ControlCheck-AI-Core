...
class MappedSnapshot:
    ...
    domain_issues: dict[str, tuple[RowIssue, ...]] = dataclass_field(
...
def _decimal(value: Any, field: str) -> Decimal:
    ...
    try:
        ...
        raise _ScalarError(
            "invalid_decimal", f"{field} is not a valid decimal"
        ) from None
...
def _date(value: Any, field: str) -> date:
    ...
    if isinstance(value, (int, float, Decimal)):
        try:
            ...
            raise _ScalarError(
                "invalid_date", f"{field} is not a valid Excel date"
            ) from None
    ...
    try:
        ...
        try:
            ...
            raise _ScalarError(
                "invalid_date", f"{field} is not a valid ISO date"
            ) from None
...
def _normalize_scalar(
    ...
    if column.scalar_type == "decimal":
        ...
        if percentage_text and column.normalization != "percentage_to_decimal":
            raise _ScalarError(
                "invalid_decimal", f"{field} does not accept percentage syntax"
            )
        decimal_value = _decimal(
            value.strip()[:-1] if percentage_text else value, field
        )
    ...
    if column.scalar_type == "integer":
        ...
        try:
            ...
            raise _ScalarError(
                "invalid_integer", f"{field} is not a valid integer"
            ) from None
        if (
    ...
    raise _ScalarError(
        "unsupported_scalar_type", f"{field} has an unsupported scalar type"
    )


def _sort_issues(
    issues: list[RowIssue], field_order: dict[str, int]
) -> tuple[RowIssue, ...]:
...
def _map_row(row: ExtractedRow, profile: MappingProfileV1) -> CanonicalRowResult:
    ...
    if row.domain == "schedule" and not any(
        ...
        if (
            ...
            and baseline_finish is not None
            and baseline_finish < baseline_start
        ):
            issues.append(
                _issue(
                    "contradictory_dates",
    ...
    if row.domain == "wbs" and not any(
        ...
        if values.get("parent_wbs") == wbs_code or values.get("level", 0) < 1:
            field = (
                "parent_wbs"
                if values.get("parent_wbs") == wbs_code
                else "level"
            )
            issues.append(
                _issue(
    ...
    values["source"] = SourceRef(
        sheet=row.sheet_name, row_number=row.source_row_number
    )
    try:
...
def _validate_wbs_rows(
    rows: list[CanonicalRowResult], profile: MappingProfileV1
    ...
    for row in result:
        if (
            not isinstance(row.record, WBSNode)
            or row.record.wbs_code not in invalid_codes
        ):
...
def map_extracted_workbook(
    extracted: ExtractedWorkbook, profile: MappingProfileV1
) -> MappedSnapshot:
    """Normalize governed raw rows into domain models without persistence."""
    rows_by_domain = {
        domain: [
            _map_row(row, profile)
            for row in extracted.rows_by_domain.get(domain, [])
        ]
    ...
    if (
        not extracted.rows_by_domain.get("wbs")
        and not template_errors_by_domain["wbs"]
    ):
        domain_issues["wbs"] = (
            _issue(
                "empty_wbs_master",
    ...
    for domain, rows in rows_by_domain.items():
        if (
            template_errors_by_domain[domain]
            or domain_issues[domain]
            or any(
