...
def _unique_value_keys(headers: tuple[Any, ...]) -> list[str]:
    literal_headers = {
        header for value in headers if (header := _header(value)) is not None
    }
...
def _header_issues(
    ...
    expected_headers = {column.source_header for column in profile.columns.values()}
    required_headers = {
    ...
    try:
        ...
        for domain, domain_profile in profile.domains.items():
            ...
            for source_row_number, row in enumerate(
                sheet.iter_rows(
                    min_row=domain_profile.header_row + 1,
                    values_only=True,
                ),
                ...
                if not any(value is not None for value in row):
                    continue
                values = {key: _json_safe(value) for key, value in zip(keys, row)}
                rows.append(
                    ExtractedRow(
                        domain=domain,
                        sheet_name=sheet_name,
