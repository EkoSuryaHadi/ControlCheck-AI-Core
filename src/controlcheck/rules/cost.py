...
class ActualExceedsBudgetRule(BaseRule):
    ...
    def evaluate(self, dataset, context):
        ...
        for wbs, budget in budgets.items():
            ...
            if actual <= budget:
                continue
            variance, ratio = actual - budget, actual / budget if budget else Decimal("0")
...
class CostSpikeRule(BaseRule):
    ...
    def evaluate(self, dataset, context):
        ...
        for wbs, periods in monthly.items():
            ...
            threshold = average * Decimal(str(context.thresholds.cost_spike_multiplier))
            materiality = project_budget * Decimal(str(context.thresholds.cost_spike_materiality_bac))
...
class VendorConcentrationRule(BaseRule):
    ...
    def evaluate(self, dataset, context):
        ...
        for vendor, (share, wbs, amount) in strongest.items():
            if share < Decimal(str(context.thresholds.vendor_concentration_warning)):
                continue
            records = [x for x in dataset.actual_costs if x.wbs_code == wbs and x.vendor_id == vendor]
            findings.append(self.finding(
                dataset, context, entity_type="vendor", entity_id=vendor,
                severity="critical" if share >= Decimal(str(context.thresholds.vendor_concentration_critical)) else "warning",
                ...
                calculation={"formula": "vendor_wbs_actual / wbs_actual >= threshold", "value": share, "threshold": context.thresholds.vendor_concentration_warning, "result": True},
            ))
        return findings
...
class HighValueTransactionRule(BaseRule):
    ...
    def evaluate(self, dataset, context):
        ...
        for item in dataset.actual_costs:
            ...
            if wbs_share < Decimal(str(context.thresholds.transaction_wbs_share)) and project_share < Decimal(str(context.thresholds.transaction_project_share)):
                continue
            findings.append(self.finding(
                dataset, context, entity_type="transaction", entity_id=item.transaction_id,
                severity="critical" if wbs_share >= Decimal("0.25") else "observation",
                ...
                calculation={"formula": "wbs_share >= threshold OR project_share >= threshold", "result": True},
            ))
        return findings
...
class HighCostLowProgressRule(BaseRule):
    ...
    def evaluate(self, dataset, context):
        ...
        for wbs, latest in progress.items():
            ...
            if cost_pct < Decimal(str(context.thresholds.cost_progress_cost_pct)) or latest.actual_progress > context.thresholds.cost_progress_max_progress:
                continue
            transactions = [x for x in dataset.actual_costs if x.wbs_code == wbs]
            findings.append(self.finding(
                dataset, context, entity_type="wbs", entity_id=wbs,
