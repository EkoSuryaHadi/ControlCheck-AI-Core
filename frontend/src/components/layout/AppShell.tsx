        ...
        <div className="flex flex-col h-full overflow-hidden">
          ...
          <nav className="flex-1 px-3 py-2 space-y-1 overflow-y-auto">
            {navItems.map((item) => {
              ...
              return (
                <NavLink key={item.name} to={item.path} className={cn("flex items-center gap-3 px-3 py-2.5 rounded-lg text-xs font-medium transition-all duration-150 group", isActive ? "bg-blue-600 text-white font-semibold shadow-sm" : "text-slate-300 hover:bg-navy-900 hover:text-white", item.isAI && !isActive && "hover:text-purple-300") }>
                  <Icon className={cn("w-4 h-4 transition-colors", isActive ? "text-white" : item.isAI ? "text-purple-400 group-hover:text-purple-300" : "text-slate-400 group-hover:text-white")} />
                  <span>{item.name}</span>
                  {item.isAI && <Sparkles className="w-3 h-3 ml-auto text-purple-400" />}
                </NavLink>
              )
            })}
          </nav>
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden bg-slate-50">
        <header className="h-14 bg-white border-b border-slate-200 px-6 flex items-center justify-between shrink-0 z-20">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => navigate("/data")} className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-semibold shadow-sm transition-colors"><Plus className="w-3.5 h-3.5" /><span>Import Dataset</span></button>
          </div>
        </header>
