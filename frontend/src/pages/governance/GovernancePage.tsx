...
export const GovernancePage: React.FC = () => {
  ...
  const decide = async (approval: ClosureApproval, decision: "approved" | "rejected") => {
    ...
    try {
      ...
      setMessage(decision === "approved" ? "Closure request approved." : "Closure request returned for revision.")
      await load()
    } catch (error: any) {
  ...
  if (!currentProject) {
    return <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center text-sm text-slate-500">Select a project to open Governance Center.</div>
  }
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 pb-12">
      <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
        <div>
          <div className="text-xs font-bold uppercase tracking-wider text-indigo-600">Manager Oversight</div>
          <h1 className="mt-1 text-2xl font-bold tracking-tight text-slate-900">Governance Center</h1>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-500">Review closure approvals, overdue findings, escalations and SLA exceptions for {currentProject.name}. Project Controllers resolve findings from the Finding Detail page; this workspace is for independent management oversight.</p>
        </div>
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Metric label="Needs Your Approval" value={approvals.length} icon={ShieldCheck} />
        <Metric label="Open Escalations" value={openEscalations.length} icon={AlertTriangle} />
        <Metric label="Critical Exceptions" value={criticalEscalations.length} icon={Clock3} />
      ...
      <section className="rounded-2xl border border-indigo-200 bg-white p-6 shadow-sm">
        <div className="flex items-center justify-between gap-3">
          <div><div className="text-xs font-bold uppercase tracking-wider text-indigo-600">Approval Inbox</div><h2 className="mt-1 text-lg font-bold text-slate-900">Findings waiting for independent review</h2><p className="mt-1 text-xs leading-5 text-slate-500">Open the finding, review its evidence and completed actions, then approve closure or return it for revision.</p></div>
          <span className="rounded-full bg-indigo-50 px-3 py-1 text-xs font-bold text-indigo-700">{approvals.length} waiting</span>
        </div>
        <div className="mt-5 grid gap-3 lg:grid-cols-2">
          {approvals.length === 0 ? <div className="lg:col-span-2"><Empty text="No finding is waiting for approval." /></div> : approvals.map((approval) => (
            <article key={approval.id} className="rounded-xl border border-slate-200 bg-slate-50/60 p-4">
              <div className="flex items-start justify-between gap-3">
                <div><div className="text-[10px] font-black uppercase tracking-wide text-slate-400">Closure Review</div><button onClick={() => navigate(`/findings/${approval.finding_id}`)} className="mt-1 font-mono text-xs font-bold text-blue-600 hover:underline">Finding {approval.finding_id}</button></div>
                <span className="rounded-full bg-amber-100 px-2 py-1 text-[10px] font-black uppercase text-amber-700">Pending</span>
              </div>
              <div className="mt-2 text-xs text-slate-500">Requested {new Date(approval.requested_at).toLocaleString()}</div>
              {approval.decision_note && <p className="mt-2 text-xs leading-5 text-slate-600">{approval.decision_note}</p>}
              <button onClick={() => navigate(`/findings/${approval.finding_id}`)} className="mt-3 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50">Review Finding & Evidence</button>
              <div className="mt-2 grid grid-cols-2 gap-2">
                ...
                <button onClick={() => decide(approval, "rejected")} className="inline-flex items-center justify-center gap-1.5 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs font-bold text-red-700 hover:bg-red-100"><XCircle className="h-4 w-4" /> Return</button>
              </div>
            </article>
          ))}
        </div>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-slate-500"><SlidersHorizontal className="h-4 w-4" /> Governance Policy</div>
        <p className="mt-2 text-xs leading-5 text-slate-500">Administrative settings for closure approval and severity-based SLA. These settings support the workflow; they are not part of the Project Controller's normal resolution steps.</p>
        ...
        <button onClick={savePolicy} disabled={saving} className="mt-5 rounded-xl bg-slate-900 px-5 py-3 text-xs font-bold text-white hover:bg-slate-800 disabled:opacity-50">{saving ? "Saving..." : "Save Governance Policy"}</button>
