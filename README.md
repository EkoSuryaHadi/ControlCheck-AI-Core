...
This branch is the public-beta release candidate. It combines the React application and FastAPI API in one Vercel project, retains durable data in Supabase PostgreSQL and private Cloudflare R2, and keeps full authentication/RBAC hardening, payment/subscription, enterprise SSO, and production-scale HA/DR deferred.

---

## Public Beta Cloud Deployment Contract

The public-beta architecture is **browser → Vercel React frontend → Vercel FastAPI Function → Supabase PostgreSQL + private Cloudflare R2**. This **Vercel React + FastAPI** topology uses same-origin `/api` routing; do not set `VITE_API_BASE_URL` for the beta deployment.

Provider responsibilities are deliberately narrow:

- **Vercel:** builds the React frontend, serves it at the exact beta origin `https://control-check-ai-git-codex-public-8a91d7-ekosuryahadis-projects.vercel.app`, and runs the FastAPI Function behind same-origin `/api` with a 240-second target duration.
